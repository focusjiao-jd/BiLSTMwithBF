...
def batch_matmul_bias(seq, weight, bias, nonlinearity=''):
    ...
    return s.squeeze()
# mind the permutation of params in gru in pytorch


class BFcombined(nn.Module):
    def __init__(self, config):
        super(BFcombined, self).__init__()
        self.bsize = config['bsize']
        self.enc_dim = config['enc_lstm_dim']
        self.dpout_model = config['dpout_model']

        # self.init_lstm = Variable(torch.FloatTensor(2, self.bsize,
        #                                             self.enc_lstm_dim).zero_()).cuda()
        self.gate = nn.Linear(2 * self.enc_dim, 2 * self.enc_dim)

    def forward(self, en_sent):
        return self.gate(en_sent)
class BFbalanced(nn.Module):
    def __init__(self, config):
        super(BFbalanced, self).__init__()
        self.bsize = config['bsize']
        self.enc_dim = config['enc_lstm_dim']
        self.dpout_model = config['dpout_model']
        ...
        # self.init_lstm = Variable(torch.FloatTensor(2, self.bsize,
        #                                             self.enc_lstm_dim).zero_()).cuda()
        self.gate = nn.Linear(self.enc_dim, self.enc_dim, False)
        # self.v = nn.Parameter(torch.Tensor(self.enc_dim, 1))
        # self.v.data.uniform_(-0.1, 0.1)
    def forward(self, en_sent):
        b_sent,f_sent = en_sent.split(self.enc_dim,2)
        # eb_sent =torch.tanh(self.gate(b_sent))
        # ef_sent =torch.tanh(self.gate(f_sent))
        eb_sent = self.gate(b_sent)
        ef_sent = self.gate(f_sent)
        mean_sent = (b_sent + f_sent) / 2
        eb_sent =torch.mul(eb_sent, mean_sent).sum(2)
        ef_sent = torch.mul(ef_sent, mean_sent).sum(2)
        a_b=eb_sent.exp()
        a_f=ef_sent.exp()
        att = a_b/(a_b+a_f)
        att =att.unsqueeze(2)
        # multiply 2?
        sent =torch.cat((b_sent*att*2 ,f_sent*(1.0-att)*2),2)
        return sent
class SGCN(nn.Module):
    def __init__(self, config):
        super(SGCN, self).__init__()
        self.bsize = config['bsize']
        self.enc_dim = 2*config['enc_lstm_dim']
        self.dpout_model = config['dpout_model']

        # self.init_lstm = Variable(torch.FloatTensor(2, self.bsize,
        #                                             self.enc_lstm_dim).zero_()).cuda()
        self.W_in = nn.Parameter(torch.Tensor(self.enc_dim, self.enc_dim))
        self.W_out = nn.Parameter(torch.Tensor(self.enc_dim, self.enc_dim))
        self.W_self = nn.Parameter(torch.Tensor(self.enc_dim, self.enc_dim))
        self.bias_in = nn.Parameter(torch.Tensor(self.enc_dim, 1))
        self.bias_out = nn.Parameter(torch.Tensor(self.enc_dim, 1))
        self.bias_self = nn.Parameter(torch.Tensor(self.enc_dim, 1))
        self.W_in.data.uniform_(-0.1, 0.1)
        self.W_out.data.uniform_(-0.1, 0.1)
        self.W_self.data.uniform_(-0.1, 0.1)
    def forward(self, en_sent, adj_tuple):
        # sent_len: [max_len, ..., min_len] (batch)
        # sent: Variable(seqlen x batch x worddim)
        en_sent =en_sent.transpose(0,1)
        adj_in, adj_out, adj_mask =adj_tuple
        en_sent =torch.nn.Dropout(p=self.dpout_model)(en_sent)
        sent_in = batch_matmul_bias(en_sent, self.W_in, self.bias_in, nonlinearity='sigmoid')
        sent_out = batch_matmul_bias(en_sent, self.W_out, self.bias_out, nonlinearity='sigmoid')
        sent_self = batch_matmul_bias(en_sent, self.W_self, self.bias_self, nonlinearity='sigmoid')
        en_sent =torch.bmm(adj_in,sent_in) + torch.bmm(adj_out,sent_out) + sent_self
        en_sent = torch.mul(adj_mask.expand_as(en_sent), en_sent)
        en_sent = en_sent.transpose(0, 1)
        return en_sent
...
class BLSTMEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.bf == 1:
            self.bflayer =BFbalanced(config)
        elif self.bf == 2:
            self.bflayer = BFcombined(config)
    def is_cuda(self):
    ...
    def forward(self, sent_tuple):
        ...
        if self.bf:
            sent_output =self.bflayer(sent_output)
        # Pooling
